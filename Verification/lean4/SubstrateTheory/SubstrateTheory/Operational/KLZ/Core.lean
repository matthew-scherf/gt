...
axiom KLZState : Type
axiom join  : List KLZState → KLZState
noncomputable axiom mode : KLZState → KLZState
noncomputable axiom K_LZ : KLZState → ℕ

axiom c_sub : ℝ
axiom c_single : ℝ
axiom C_mode : ℝ
axiom K_LZ_nonneg (s : KLZState) : 0 ≤ K_LZ s
axiom K_LZ_empty : K_LZ (join []) = 0
axiom K_LZ_subadditive_cons
  (x : KLZState) (xs : List KLZState) :
  (K_LZ (join (x :: xs)) : ℝ) ≤ (K_LZ (join xs) : ℝ) + (K_LZ x : ℝ) + c_sub

axiom K_LZ_prefix
  (b : KLZState) (s : List KLZState) :
  K_LZ (join [b]) ≤ K_LZ (join (b :: s))

axiom K_LZ_singleton_bound : ∀ b : KLZState, (K_LZ (join [b]) : ℝ) ≤ c_single
axiom K_LZ_mode_le (s : KLZState) :
  (K_LZ (mode s) : ℝ) ≤ (K_LZ s : ℝ) + C_mode

axiom C_mode_lt_c_grounding : C_mode < c_grounding

theorem R_G1_preserves_grounding (n : List KLZState) :
